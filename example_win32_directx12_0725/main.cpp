...
//----------------------------MY RESOURCE MANAGEMENT------------------------------------------------
            ...
            ImGui::Checkbox("notes", &show_another_window);
            ...
            //   ImGui::ShowDemoWindow(&show_demo_window);
        {
            ...
            if (!ImGui::CollapsingHeader("Template"))
            {
                if (ImGui::TreeNode("LZY_01"))
                {
                    ...
                    bool ret = LoadTextureFromFile("1.jpg", g_pd3dDevice, my_texture_srv_cpu_handle, &my_texture, &my_image_width, &my_image_height);
                    IM_ASSERT(ret);
                    ...
                    // ImGui::Text("size = %d x %d", my_image_width, my_image_height);
                     ...
                     //ImGui::Image((ImTextureID)my_texture_srv_gpu_handle.ptr, ImVec2((float)my_image_width*2, (float)my_image_height*0.5));
                    //ImGui::Text("GPU=%p", my_texture_srv_gpu_handle.ptr);
                    ...
                    if(ImGui::Begin("Show"));
                        {
                           ImGui::Text("My Resource Management.");

                        }
                    ImGui::End();             
                    ImGui::TreePop();
                }
                if (ImGui::TreeNode("LZY_02"))
                {
                    ImGuiIO& io = ImGui::GetIO();
                    ImGui::SetNextWindowBgAlpha(0);
                    // ImGui::SetNextWindowSize(ImVec2(io.DisplaySize.x, io.DisplaySize.y));
                    ...
                    D3D12_GPU_DESCRIPTOR_HANDLE my_texture_srv_gpu_handle = g_pd3dSrvDescHeap->GetGPUDescriptorHandleForHeapStart();
                    my_texture_srv_gpu_handle.ptr += (handle_increment * descriptor_index);
                    bool ret = LoadTextureFromFile("2.jpg", g_pd3dDevice, my_texture_srv_cpu_handle, &my_texture, &my_image_width, &my_image_height);
                    IM_ASSERT(ret);

                    //  my_texture_srv_gpu_handle_temp.ptr = my_texture_srv_gpu_handle.ptr;
                    // ImGui::Text("CPU handle = %p", my_texture_srv_cpu_handle.ptr);
                    // ImGui::Text("GPU handle = %p", my_texture_srv_gpu_handle.ptr);
                    // ImGui::Text("size = %d x %d", my_image_width, my_image_height);
                     // Note that we pass the GPU SRV handle here, *not* the CPU handle. We're passing the internal pointer value, cast to an ImTextureID
        ...
        {
            ImGui::Begin("My test Demo", &show_another_window);   // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
...
bool CreateDeviceD3D(HWND hWnd)
{
...

    {
        ...
        desc.NumDescriptors = 3;//set to 2
